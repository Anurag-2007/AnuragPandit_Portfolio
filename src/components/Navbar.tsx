...
const navItems: NavItem[] = [
  { label: 'Work', hash: '#work', isExternal: false },
  { label: 'Experience', hash: '#experience', isExternal: false },
  { 
    label: 'Resume', 
    href: 'https://docs.google.com/document/d/1gYlnRoRzAJ4a8jC-AcPOwY0A7dBMXaaQgh-kk1bf1q8/edit?tab=t.0', 
    isExternal: true 
  },
  { label: 'Photography', path: '/photography', icon: Camera, isExternal: false },
];
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-3 md:pt-6 px-3 pointer-events-none">
      <div className={`pointer-events-auto inline-flex items-center gap-1 sm:gap-2.5 rounded-full backdrop-blur-2xl border border-white/10 bg-surface/80 p-1 sm:px-3.5 sm:py-2 transition-all duration-500 shadow-2xl ${scrolled ? 'shadow-black/60 border-white/20 bg-surface/90' : ''}`}>
        ...
        <button 
          onClick={() => handleHashNav('#home')}
          className="relative group p-0.5 flex items-center justify-center cursor-pointer shrink-0 bg-transparent border-none"
        >
        ...
        <div className="flex items-center gap-0.5 sm:gap-1">
          {navItems.map((item, index) => {
            ...
            const commonClasses = "relative group flex items-center gap-1.5 text-[10px] sm:text-xs rounded-full px-2 sm:px-3 py-1 sm:py-1.5 text-muted hover:text-text-primary hover:bg-white/10 hover:shadow-[0_0_15px_rgba(137,170,204,0.15)] transition-all duration-300 cursor-pointer";
        ...
        <button 
          ...
          className="relative group text-[10px] sm:text-xs rounded-full px-2 sm:px-3 py-1 sm:py-1.5 text-text-primary inline-flex items-center justify-center overflow-hidden shrink-0 cursor-pointer bg-transparent border-none"
        >
