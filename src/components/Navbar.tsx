  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-4 md:pt-6 px-4 pointer-events-none">
      <div className={`pointer-events-auto inline-flex items-center gap-1.5 sm:gap-2.5 rounded-full backdrop-blur-2xl border border-white/10 bg-surface/80 p-1.5 sm:px-3.5 sm:py-2 transition-all duration-500 shadow-2xl ${scrolled ? 'shadow-black/60 border-white/20 bg-surface/90' : ''}`}>
        ...
        {/* Links with Subtle Separating Lines & Text on Mobile/Desktop */}
        <div className="flex items-center gap-0.5 sm:gap-1">
          {navItems.map((item, index) => {
            const commonClasses = "relative group flex items-center text-[11px] sm:text-xs rounded-full px-2.5 sm:px-4 py-1.5 text-muted hover:text-text-primary hover:bg-white/10 hover:shadow-[0_0_15px_rgba(137,170,204,0.15)] transition-all duration-300";
        ...
        <a 
          href="#contact" 
          className="relative group text-[11px] sm:text-xs rounded-full px-2.5 sm:px-3 py-1.5 text-text-primary inline-flex items-center justify-center overflow-hidden shrink-0"
        >
          ...
          <div className="relative flex items-center gap-1 bg-black/50 border border-white/10 rounded-full px-3 py-1.5 backdrop-blur-md z-10 group-hover:border-[#89AACC]/50 group-hover:bg-surface transition-all duration-300">
            <span>Say hi</span>
